...
- (UIView *)customTableViewCell_Dinner
{
    ...
    comment_Dinner.image = [UIImage imageNamed:@"POIRatingView_Star_Foreground@2x.png"];
    ...
    // 距离
    distanceLabel_Dinner = [[UILabel alloc] initWithFrame:kFrame(kScreenWidth - 10 - 80, 65, 80, 15)];
...
- (UIButton *)packageButtonWithImage:(UIImage *)image Title:(NSString *)title Frame:(CGRect)frame
{
    ...
    // 添加点击的方法
    [navgationBtn addTarget:self action:@selector(navigationClicksAction:) forControlEvents:UIControlEventTouchUpInside];
    // 设置tag值
    navgationBtn.tag = 1020 + flag;
